...
const SearchBar = ({ setMovies, setMoviesToShow, movies, moviesToShow }) => {
  ...
  const filter = () => {
    let moviesFiltered = {
      none: "No encontramos una pelicula con esa puntuación",
    };
    if (movies.results) {
      moviesFiltered = movies.results
        .filter((movie) => {
          return (
            movie.vote_average <= rateValue &&
            movie.vote_average > rateValue - 2
          );
        })
        .sort((movieA, movieB) => movieB.vote_average - movieA.vote_average);
    }
    if (moviesFiltered && moviesFiltered.length === 0) {
      return setMoviesToShow({
        none: "No encontramos una pelicula con esa puntuación",
      });
    }
    ...
    <section className="search-filter-container">
      ...
      <form className="clasification-filter">
        ...
        <div className="stars-container">
          ...
          />
          <label htmlFor="rate-1" className="rate-label">
            {rateValue > 0 ? <AiFillStar /> : <AiOutlineStar />}
          </label>
          <input
            className="rate-input"
            type="radio"
            ...
            onClick={changeRateValue}
          />
          <label htmlFor="rate-2" className="rate-label">
            {rateValue > 2 ? <AiFillStar /> : <AiOutlineStar />}
          </label>
          <input
            className="rate-input"
            type="radio"
            ...
            onClick={changeRateValue}
          />
          <label htmlFor="rate-3" className="rate-label">
            {rateValue > 4 ? <AiFillStar /> : <AiOutlineStar />}
          </label>
          <input
            className="rate-input"
            type="radio"
            ...
            onClick={changeRateValue}
          />
          <label htmlFor="rate-4" className="rate-label">
            {rateValue > 6 ? <AiFillStar /> : <AiOutlineStar />}
          </label>
          <input
            className="rate-input"
            type="radio"
            ...
            onClick={changeRateValue}
          />
          <label htmlFor="rate-5" className="rate-label">
            {rateValue > 8 ? <AiFillStar /> : <AiOutlineStar />}
          </label>
        </div>
      </form>
    </section>
